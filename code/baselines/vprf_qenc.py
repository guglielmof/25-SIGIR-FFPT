...
if __name__ == "__main__":
    ...
    run_feedback = local_utils.retrieve.retrieve_faiss(qembs, q2r, indexWrapper)
    run_feedback["rank"] = run_feedback.groupby("query_id")["score"].rank(ascending=False)
    run_feedback = run_feedback[run_feedback["rank"] <= 20].merge(docs)
    run_feedback["representation"] = list(encoder.encode_queries(run_feedback.text))

    def vprf_rocchio(query, run, k=5, alpha=0.4, beta=0.6):
        local_run = run.loc[(run.query_id == query.query_id) & (run["rank"] <= k)]
        out = alpha * query.representation + beta * np.vstack(local_run.representation).mean(axis=0)
        return out

    queries["vprf_representation"] = queries.apply(lambda x: vprf_rocchio(x, run_feedback, k=args.k), axis=1)

    qembs = np.array(queries.vprf_representation.to_list())
    q2r = pd.DataFrame(enumerate(queries.query_id.to_list()), columns=["row", "query_id"])
    run = local_utils.retrieve.retrieve_faiss(qembs, q2r, indexWrapper)
    out = compute_measure(run, qrels, ["AP", "nDCG@20", "nDCG@10", "nDCG@100", "nDCG@50"])
    out.to_csv(f"data/performance/vprf{args.k}_{args.collection}_{args.encoder}.csv", index=False)
